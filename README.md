...
### Build a self-contained tar.gz package (x86_64)

This uses the AppImage container build to bundle the application, CEF, and user-space libraries. Run:

```bash
./packaging/tar/tar.sh
tar -xzf dist/linux-wallpaperengine-1.0.0-x86_64.tar.gz
./linux-wallpaperengine.AppDir/linux-wallpaperengine [options] <background_id or path>
```

The extracted directory is portable. Like the AppImage, it uses the host kernel, graphics drivers, display server, and audio service.
